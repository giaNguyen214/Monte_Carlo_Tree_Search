...
def evaluate_position(board_fen):
    ...
    for square, piece in pieces.items():
        ...
        if piece.piece_type == chess.PAWN:
            ...
            bonus = rank - 1 if piece.color == chess.WHITE else 6 - rank
            value += bonus * 0.1
...
def mcts(board, iter_limit=1000, time_limit=20.0, c_param=1.4):
    ...
    while iterations < len(list(board.legal_moves))**iter_limit:
        ...
        if node:
            ...
            temp_node_board = node.board.copy()
            # Backpropagation
            while node:
                node.update(result if node.board.turn == temp_node_board.turn else  -result)
                node = node.parent
    ...
    best_child = max(root.children, key=lambda n: n.wins/n.visits + root.difficulty * math.sqrt(math.log(root.visits) / n.visits)  if n.visits > 0 else 0)
